...
На основе данных DWH подготовлены SQL-витрины для анализа выдач, состояния кредитного портфеля, просроченной задолженности, РППУ, FPD, Vintage и Roll Rate. На основе подготовленных данных разработан дашборд в Power BI для визуализации и анализа результатов.

## Данные

В основе проекта использованы данные кредитного портфеля банка. Для публикации проекта в открытом доступе числовые показатели были изменены и не соответствуют фактическим значениям. Персональные данные клиентов и сотрудников в проекте не используются.
...
Витрина для анализа текущего состояния кредитного портфеля на выбранную дату с детализацией по остаткам основного долга, процентам, просроченной задолженности, статусу клиента, филиалам и отделениям.

### Анализ PAR

Анализ просроченной задолженности по категориям PAR 1, PAR 30 и PAR 90 с оценкой динамики изменения качества кредитного портфеля.
